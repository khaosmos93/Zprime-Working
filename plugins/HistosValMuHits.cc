...
class HistosValMuHits : public edm::EDAnalyzer {
 public:
  explicit HistosValMuHits(const edm::ParameterSet&);
 ...
 private:
  ...
  void fillValMuHitsHistos(const pat::Muon*);
  ...
  TH1F* NBeamSpot;
  TH1F* NVertices;
  TH1F* NVerticesPassingSelection;
  TH1F* NLeptons;
  TH2F* LeptonSigns;
  TH1F* LeptonEta;
  TH1F* LeptonRap;
  ...
  TH2F* ValMu_RelTkIsoPt;
  TH2F* ValMu_RelTkIsoAbsP;
  TH2F* ValMu_RelTkIsoEta;
};

HistosValMuHits::HistosValMuHits(const edm::ParameterSet& cfg)
    ...
    _madgraphWeight(1.)
{
  ...
  //-- Val mu hits studies
  ValMu_EtaPt        = fs->make<TH2F>("ValMu_EtaPt",   "", 100, -5, 5, 5000, 0, 5000);
  ValMu_EtaAbsP      = fs->make<TH2F>("ValMu_EtaAbsP", "", 100, -5, 5, 5000, 0, 5000);
  ValMu_EtaPhi       = fs->make<TH2F>("ValMu_EtaPhi",  "", 100, -5, 5, 100, -TMath::Pi(), TMath::Pi());

  ValMu_RelTkIsoPt   = fs->make<TH2F>("ValMu_RelTkIsoPt",   "", 500, 0, 5, 5000, 0, 5000);
  ValMu_RelTkIsoAbsP = fs->make<TH2F>("ValMu_RelTkIsoAbsP", "", 500, 0, 5, 5000, 0, 5000);
  ValMu_RelTkIsoEta  = fs->make<TH2F>("ValMu_RelTkIsoEta",  "", 500, 0, 5, 100, -5, 5);
  ...
  NVerticesPassingSelection = fs->make<TH1F>("NVerticesPassingSelection", titlePrefix + "# vertices/event",  200, 0, 200);
...
void HistosValMuHits::getBSandPV(const edm::Event& event) {
  ...
  nvtx = vertex_count;
  NVertices->Fill(nvtx, _madgraphWeight  );
}

void HistosValMuHits::fillBasicLeptonHistos(const reco::CandidateBaseRef& lep) {
...
void HistosValMuHits::fillOfflineMuonHistos(const pat::Muon* mu) {
...
void HistosValMuHits::fillValMuHitsHistos(const pat::Muon* mu) {

  const reco::TrackRef track = patmuon::getPickedTrack(*mu);
  if (track.isAvailable()) {
    const reco::MuonIsolation& iso03 = mu->isolationR03();
    float relIsoSumPt = iso03.sumPt / mu->innerTrack()->pt();

    ValMu_EtaPt->Fill(   track->eta() , track->pt()  , _madgraphWeight  );
    ValMu_EtaAbsP->Fill( track->eta() , track->p()   , _madgraphWeight  );
    ValMu_EtaPhi->Fill(  track->eta() , track->phi() , _madgraphWeight  );

    ValMu_RelTkIsoPt->Fill(   relIsoSumPt , track->pt()  , _madgraphWeight  );
    ValMu_RelTkIsoAbsP->Fill( relIsoSumPt , track->p()   , _madgraphWeight  );
    ValMu_RelTkIsoEta->Fill(  relIsoSumPt , track->eta() , _madgraphWeight  );

  }
}

void HistosValMuHits::fillOfflineElectronHistos(const pat::Electron* lep) {
  // Can add electron quantities here.
}

void HistosValMuHits::fillLeptonHistos(const reco::CandidateBaseRef& lep) {
  fillBasicLeptonHistos(lep);
  
  const pat::Muon* muon = toConcretePtr<pat::Muon>(lep);
  if (muon) fillOfflineMuonHistos(muon);
  if (muon) fillValMuHitsHistos(muon);
  
  const pat::Electron* electron = toConcretePtr<pat::Electron>(lep);
  if (electron) fillOfflineElectronHistos(electron);
}

void HistosValMuHits::fillLeptonHistos(const edm::View<reco::Candidate>& leptons) {
...
void HistosValMuHits::fillLeptonHistosFromDileptons(const pat::CompositeCandidateCollection& dileptons) {
  int nleptons = 0;
  int total_q = 0;
...
void HistosValMuHits::fillDileptonHistos(const pat::CompositeCandidate& dil, const edm::Event& event) {
...
void HistosValMuHits::fillDileptonHistos(const pat::CompositeCandidateCollection& dileptons, const edm::Event& event) {
  NDileptons->Fill(dileptons.size(), _madgraphWeight  );
  if( dileptons.size()>0 )  NVerticesPassingSelection->Fill(nvtx, _madgraphWeight  );
...
void HistosValMuHits::analyze(const edm::Event& event, const edm::EventSetup& setup) {
  if (dbg_tree) {
...
DEFINE_FWK_MODULE(HistosValMuHits);
